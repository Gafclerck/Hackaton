...
Each status has a background and text color pair:

| Status | Background Token | Text Token |
|--------|-----------------|------------|
| `nouveau` | `--status-nouveau-bg` (#E5E7EB) | `--status-nouveau-text` (#4B5563) |
| `en_analyse_ia` | `--status-ia-bg` (#9C7A3C) | `--status-ia-text` (#ffffff) |
| `en_attente` | `--status-attente-bg` (#FEF3C7) | `--status-attente-text` (#92400E) |
| `affecte` | `--status-affecte-bg` (#DBEAFE) | `--status-affecte-text` (#1B2A4A) |
| `en_cours` | `--status-en-cours-bg` (#D1FAE5) | `--status-en-cours-text` (#064E3B) |
| `transfert_demande` | `--status-transfert-bg` (#FDE68A) | `--status-transfert-text` (#78350F) |
| `cloture` | `--status-cloture-bg` (#374151) | `--status-cloture-text` (#F9FAFB) |
| `archive` | `--status-archive-bg` (#F3F4F6) | `--status-archive-text` (#9CA3AF) |
...
| POST | `/api/agence/agence` | None* | `{ nom, adresse, ville, telephone, est_siege, actif }` | `AgenceResponse` |
| GET | `/api/agence/agence/{id}` | None* | — | `AgenceResponse` |
| GET | `/api/agence/agences` | None* | — | `list[AgenceResponse]` |
| PATCH | `/api/agence/agence/{id}` | None* | `AgenceRequest` | `AgenceResponse` |
| GET | `/api/type_affaires` | Bearer | — | `list[TypeAffaireRead]` |
| POST | `/api/type_affaires/create` | Bearer | `{ libelle }` | `TypeAffaireRead` |
| GET | `/api/specialites` | Bearer | — | `list[SpecialiteRead]` |
| POST | `/api/specialites/create` | Bearer | `{ libelle, description? }` | `SpecialiteRead` |

*Note: Agence routes currently have no auth guard (backend security gap).
...
|-------|-------|------------|----------------|
| User | `user` | id, nom, prenom, email, password_hash, role, agence_id, actif | Auth only |
| Agence | `agence` | id, nom, adresse, ville, telephone, est_siege, actif | CRUD |
| Client | `client` | id, type_client, nom, telephone, email, nin, rccm | Read + Create |
| Dossier | `dossier` | id, reference, titre, statut, priorite, client_id, agence_receptrice_id, agence_assigne_id, avocat_assigne_id, type_affaire_id, date_reception, date_affectation, date_cloture, description_initiale | **NO** |
| TypeAffaire | `type_affaire` | id, libelle, code | Read + Create |
| Specialite | `specialite` | id, libelle, description | Read + Create |
| UserSpecialite | `user_specialite` | user_id, specialite_id, niveau (composite PK) | **NO** |
| Document | `document` | id, dossier_id, uploaded_by_id, nom_fichier, chemin_stockage, type_mime, taille_octets, description, confidentiel | **NO** |
| AnalyseIa | `analyse_ia` | id, dossier_id, resume_genere, type_detecte, mots_cles (JSONB), agence_suggeree_id, avocat_suggere_id, score_confiance, modele_ia, validee | **NO** |
| HistoriqueAction | `historique_action` | id, dossier_id, user_id, action, ancienne_valeur, nouvelle_valeur, commentaire | **NO** |
| Discussion | `discussion` | id, dossier_id, created_by_id, sujet | **NO** |
| MessageDiscussion | `message_discussion` | id, discussion_id, auteur_id, contenu, parent_message_id | **NO** |
| Notification | `notification` | id, destinataire_id, dossier_id, type, contenu, lue | **NO** |
...
```
nouveau → en_analyse_ia → en_attente → affecte → en_cours → cloture → archive
                                           ↑
                              transfert_demande (can loop back)
```

**Status descriptions**:
1. `nouveau` — Just received, not yet processed
2. `en_analyse_ia` — Being analyzed by AI for classification and suggestion
3. `en_attente` — AI analysis done, waiting for human assignment
4. `affecte` — Assigned to an office and lawyer
5. `en_cours` — Lawyer actively working on the case
6. `transfert_demande` — Transfer to another office requested
7. `cloture` — Case resolved and closed
8. `archive` — Stored for historical records
...
| `/clients` | PlaceholderPage | Protected | **TODO: not built** |
| `/clients/:id` | PlaceholderPage | Protected | **TODO: not built** |
| `/agences` | PlaceholderPage | Protected | **TODO: not built** |
| `/utilisateurs` | PlaceholderPage | Protected | **TODO: not built** |
| `/specialites` | PlaceholderPage | Protected | **TODO: not built** |
| `/parametres` | PlaceholderPage | Protected | **TODO: not built** |
...
- Dossier detail (tabbed: Aperçu with IA analysis + assignment info)
- File d'affectation (assignment queue with IA suggestions)
- Clients (grid/list view, create dialog, detail page)
...
- Agences CRUD page
- Utilisateurs CRUD page
- Spécialités management page
- Paramètres page
- Error handling / toast notifications
- Code splitting (bundle is 504KB)
- Real-time updates
